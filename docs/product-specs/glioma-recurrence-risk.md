...
Build a retrospective research pipeline that predicts future glioma recurrence locations from post-op pre-radiotherapy MRI plus radiotherapy dose maps.

## V1 Scope

- Inputs: `patients.csv`, DICOM T1c/FLAIR, DICOM RTDOSE, reviewed recurrence masks.
- Outputs: baseline-space NIfTI derivatives and `recurrence_risk.nii.gz`.
- Models: dose/distance baseline, voxel-sampled logistic baseline, optional MONAI/PyTorch U-Net.
